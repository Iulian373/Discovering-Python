...
def convert_shape_format(shape1):
    positions = []

    format = shape1.shape[shape1.rotation % len(shape1.shape)]

    for i, line in enumerate(format):
        row = list(line)
        for j, column in enumerate(row):
            if column == '0':
                positions.append((shape1.x + j, shape1.y + i))
...
def draw_window(surface, grid, score, last_score):
    ...
    label = font.render('Tetris', 1, (255,165,0))
...
pygame.display.set_caption('Tetris by Iulian Stratan')
